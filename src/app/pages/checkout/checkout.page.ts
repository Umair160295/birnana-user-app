...
export class CheckoutPage implements OnInit {
  ...
  async paystackPay() {
    ...
    const param = {
      ...
      ref: paykey,
      callback_url:'https://api.birnanaapp.site/api/v1/success_payments',
      metadata:{"cancel_action": "https://api.birnanaapp.site/api/v1/failed_payments"}
    }
    const header = {
      headers: new HttpHeaders()
        .set('Content-Type', 'application/json')
        .set('Authorization', `Bearer sk_test_9a74c52f7c4490ca36d096bcaa759da38d216509`)
    };
    // const param = this.JSON_to_URLEncoded(body);
    this.http.post('https://api.paystack.co/transaction/initialize', param, header).subscribe((data:any) => {
      this.util.hide();
    const browser: any = this.iab.create(data.data.authorization_url, '_blank', options);
